...
Bekannte Fehler betreffen Funktionserweiterungen. Die aktuelle Implementierung mit den aktuell definierten Parameter funktioniert nach ersten Tests Ordnungsgemäss. (Weitere unbekannte Fehler können aber nicht zu 100% ausgeschlossen werden).
...
-  Der Transfer mit einem höheren ID - Bereich (z.B 4 - Parameter TLP_ID_WIDTH)  funktioniert nicht. Der höhere ID - Bereich erzeugt ein 2 - Byte Header. Der Transmitter schreibt diesen im Sendemodus ab FIFO korrekt auf die Leitung. Im Replay - Modus zeigt sich ein Fehler. In der Receiver - Schaltung zeigt sich immer ein Fehler. Alle Zähler, respektive selektierte Bereiche in den Schieberegistern werden anhand der Anzahl TLP - Bytes ermittelt. Dies ist über Parameter festgelegt. Dieser wird bestimmt aus TLP - Bytes + Header - Bytes. Dieser Ansatz ist aber falsch! Teile der Schaltungen müssen immer TLP - Bytes + 1 haben und andere wiederum TLP-Bytes + Header-Bytes. Da die aktuelle ID - Breich bis 3 immer nur ein Byte - Header verwendet, fällt dieser Fehler nicht auf. Es müssen alle Schaltungen des Transmitters und Receivers in Bezug auf diese Parameter analysiert werden. Dies ist relevant in Bezug auf die Funktionserweiterung für variable ID - Längen über verstellbaren Parameter. Grundsätzlich funktioniert die aktuelle Implementierung für ID - Breite von 0 - 7 (Es werden maximal 8 TLP - Frames verkettet). Bei grösserer Menge von TLP's (z.B Verkettung von 16 TLP's ab FIFO) könnte die Leistung aber verbessert werden, wenn man den Fehler behebt und es ermöglicht, einen grösseren ID Bereich durch Vergrösserung von zwei Bytes an Kopfdaten erhält.  
